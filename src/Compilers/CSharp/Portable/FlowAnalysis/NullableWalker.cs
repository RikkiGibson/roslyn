        ...
        : LocalDataFlowPass<NullableWalker.LocalState, NullableWalker.LocalFunctionState>
    {
        ...
        protected override ImmutableArray<PendingBranch> Scan(ref bool badRegion)
        {
            ...
            if (!_isSpeculative)
            {
                ...
                // Tracked by https://github.com/dotnet/roslyn/issues/76130 : should register the extension parameter
        ...
        private void InheritNullableStateOfMember(int targetContainerSlot, int valueContainerSlot, Symbol member, bool isDefaultValue, int skipSlot)
        {
            ...
            {
                if ((object)_symbol == backingField)
                    return true; // we are analyzing the field initializer for this backing field.
            ...
            if (symbol is SynthesizedBackingFieldSymbol { InfersNullableAnnotation: true } backingField)
            {
                ...
                if (_getterNullResilienceData is var (analyzedField, assumedNullableAnnotation))
                {
                    if ((object)analyzedField != backingField)
                    {
                        ...
                        // In retail builds, we should proceed by simply using the non-inferred property nullability for the given backing field.
                        Debug.Assert(false);
...
#pragma warning restore IDE0051 // Remove unused private members
            ...
            TypeWithState rightType)
        {
                ...
                method?.ParameterCount == 2)
            {
                ...
                var reinferredMethod = (MethodSymbol)AsMemberOfType(asMemberOfType, method);
                SetUpdatedSymbol(binary, method, reinferredMethod);
                method = reinferredMethod;

                var parameters = method.Parameters;
                visitOperandConversionAndPostConditions(left, leftOperand, leftConversion, parameters[0], leftUnderlyingType);
                visitOperandConversionAndPostConditions(right, rightOperand, rightConversion, parameters[1], rightUnderlyingType);
        ...
        private FlowAnalysisAnnotations GetParameterAnnotations(ParameterSymbol parameter)
        {
            ...
            {
                // Tracked by https://github.com/dotnet/roslyn/issues/76130 : revisit when doing nullability analysis
                return FlowAnalysisAnnotations.None;
            }
            ...
            bool expanded)
        {
            ...
            var result = MethodTypeInferrer.Infer(
                ...
                new MethodInferenceExtensions(this)); // Tracked by https://github.com/dotnet/roslyn/issues/76130 : we may need to override ordinals here
        ...
        private static Symbol AsMemberOfType(TypeSymbol? type, Symbol symbol)
        {
            ...
            // Tracked by https://github.com/dotnet/roslyn/issues/76130 : The assert below fails for an instance call on a generic extension.
...
#nullable enable
        ...
        private void VisitDeconstructMethodArguments(ArrayBuilder<DeconstructionVariable> variables, Conversion conversion, BoundExpression right, TypeWithState? rightResultOpt)
        {
            ...
            if (deconstructMethod is object)
            {
                ...
                if (invocation.InvokedAsExtensionMethod) // Tracked by https://github.com/dotnet/roslyn/issues/76130: Do we need to do anything special for new extensions here?
                {
