...
internal sealed class FileBasedProgramsProjectSystem : LanguageServerProjectLoader, ILspMiscellaneousFilesWorkspaceProvider, IDisposable, IOnInitialized
{
    ...
    private async ValueTask<LooseDocumentKind> ClassifyDocumentAsync(DocumentUri documentUri, TrackedDocumentInfo documentInfo, LanguageInformation languageInformation, CancellationToken cancellationToken)
    {
        ...
        if (filePath is { }
            ...
            && VirtualProjectXmlProvider.HasFileBasedAppDirectives(documentInfo.SourceText))
        {
        ...
        syntaxTree ??= CSharpSyntaxTree.ParseText(documentInfo.SourceText, cancellationToken: cancellationToken);
    ...
    private async ValueTask<TextDocument?> GetOrLoadDocumentCoreAsync(TextDocumentIdentifier textDocumentIdentifier, LooseDocumentKind documentKind, TrackedDocumentInfo documentInfo, LanguageInformation languageInformation, CancellationToken cancellationToken)
    {
        ...
        else if (documentKind is LooseDocumentKind.BareMiscFile or LooseDocumentKind.RichMiscFile or LooseDocumentKind.RichMiscFileWithSemanticErrors)
        {
        ...
        async ValueTask<TextDocument> GetOrLoadFileBasedAppAsync()
        {
            ...
            var fileBasedDoc = documents.FirstOrDefault(doc => doc.Project.ParseOptions?.Features.ContainsKey("FileBasedProgram") == true);
            if (fileBasedDoc is { })
                return fileBasedDoc;
        ...
        {
            Contract.ThrowIfFalse(documentKind is LooseDocumentKind.BareMiscFile or LooseDocumentKind.RichMiscFile or LooseDocumentKind.RichMiscFileWithSemanticErrors);
            ...
            var miscDoc = documents.FirstOrDefault();
            if (miscDoc is { })
                return miscDoc;
